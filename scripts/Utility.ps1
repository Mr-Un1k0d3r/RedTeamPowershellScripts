...
function Search-EventForUserByIP{
	...
	PROCESS {
		...
		ForEach($dc in $dcs) {
			ForEach($item in $IP) {
				...
				$xmlFilter = "<QueryList><Query Id=""0"" Path=""Security""><Select Path=""Security"">*[EventData[Data[@Name=""IpAddress""] and(Data=""$($item)"")]]</Select></Query></QueryList>";
